    ...
    "solveTree" - {
      "a simple interdependent graph" - {
        TypeTraverse.solveTree(DependencyGraph(Map(
          ...
          untyped.DefDef(g('a), Some(Seq(untyped.Param("x", TypeName.Named(g('i32))))), None,
          ...
          ast.DefDef(g('a), Some(Seq(ast.Param("x", I32))), None,
      ...
      "type a def as a function type" - {
        TypeTraverse.solveTree(DependencyGraph(Map(
          ...
          untyped.DefDef(g('a), Some(Seq(untyped.Param("x", TypeName.Named(g('i32))))), None, untyped.Var(l('x))),
          untyped.ValDef(g('b), None, untyped.Var(g('a)))
        ), Ctxt.default) ==> Right(Set(
          ast.DefDef(g('a), Some(Seq(ast.Param("x", I32))), None, ast.Var(l('x)) :: I32),
      ...
      "a recursive def with type annotation" - {
        TypeTraverse.solveTree(DependencyGraph(Map(
          ...
          untyped.DefDef(g('factorial), Some(Seq(untyped.Param("i", TypeName.Named(g('i64))))), Some(TypeName.Named(g('i64))),
            untyped.App(untyped.Var(g('factorial)), Seq(untyped.Var(l('i)))))
        ), Ctxt.default) ==> Right(Set(
          ast.DefDef(g('factorial), Some(Seq(ast.Param("i", I64))), Some(TypeName.Named(g('i64))),
